  ...
  Response _responseFromCacheValue(CacheValue value, RequestOptions options) {
    ...
    if (value.headers != null) {
      headers = Headers.fromMap(
          (Map<String, List<dynamic>>.from(jsonDecode(utf8.decode(value.headers!))))
    ...
    return Response(
        data: data, headers: headers, requestOptions: options, statusCode: value.statusCode ?? 200);
  }
      ...
      if (cache != null) {
        ...
        if (maxAge == null) {
          _tryParseHead(response, (_maxAge, _staleTime) {
            maxAge = _maxAge;
            staleDate = _staleTime != null ? DateTime.now().add(_staleTime) : null;
          });
        }
