...
class _$Data<T extends IModel> extends Data<T> {
  ...
  @override
  String toString() {
    return 'CollectionModel<$T>(data: $data, pages: $pages, object: $object, url: $url, dataUpdatedAt: $dataUpdatedAt, totalCount: $totalCount)';
  }
  ...
  TResult when<TResult extends Object?>(
    ...
    return $default(data, pages, object, url, dataUpdatedAt, totalCount);
  }
...
abstract class Data<T extends IModel> extends CollectionModel<T>
    implements Collection<T>, IResponse<T> {
  const factory Data(
      {required List<ResourceModel<T>> data,
      required PagesModel pages,
      required ObjectType object,
...
abstract class Loading<T extends IModel> extends CollectionModel<T> {
...
abstract class ErrorDetails<T extends IModel> extends CollectionModel<T> {
